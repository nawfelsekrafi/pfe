...
if __name__ == "__main__":
    ...
    small_tours = tours.from_big_list_to_small_tours(students) # students[] =>> {go8: [], go10: [], back10: [], back12: []} # done 
    # it can be a function
    for k,v in sorted_tours.items(): 
        for x in dic :
            # key : x  = go8
            # value : dic[x] = [st1,st2,st3]
            sorted_tour = tours.from_small_tour_to_small_sorted_tour([st1,st2,st3]) 
            # map.map_representation(sorted_tour,bus_local) # done but still need testing 
            # recognition.run(dic[x]) # done but still need testing 
        if next continue     
    print("See You Tomorrow Sir, Bye!")
"""